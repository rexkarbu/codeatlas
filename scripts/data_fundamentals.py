...
FUNDAMENTAL_TOPICS = [
    {
        ...
        "explanation_simple": "Bayangkan kamu sedang menuliskan resep membuat secangkir kopi untuk seseorang yang belum pernah ke dapur. Instruksi 'tuang air panas, masukkan kopi bubuk, aduk' akan menghasilkan kopi yang berbeda jika urutannya dibalik menjadi 'aduk cangkir kosong, tuang air panas, buang ampas'. Komputer bekerja persis seperti asisten dapur tersebut: ia tidak memiliki inisiatif atau akal sehat untuk menebak maksud tersembunyi pembuatnya, melainkan menjalankan setiap langkah secara harfiah sesuai urutan yang tertulis.\n\nLogika pemrograman menghubungkan pemikiran analitis manusia dengan perilaku perangkat keras komputer. Analogi resep ini memiliki batas: manusia bisa berhenti saat mencium bau gosong, sedangkan program komputer akan terus menjalankan instruksi hingga selesai atau mengalami crash jika kondisi batas tidak diantisipasi sejak awal dalam kode.",
        "explanation_technical": "Logika pemrograman bertumpu pada tiga konstruksi fundamental: sequence (eksekusi baris berurutan), selection (percabangan berbasis nilai boolean), dan iteration (pengulangan berbasis kondisi terminasi). Instruksi dieksekusi oleh Program Counter (PC) di dalam CPU yang menunjuk ke alamat memori instruksi berikutnya secara sekuensial, kecuali ada instruksi jump atau branch yang mengubah nilai PC tersebut.\n\nAlgoritma adalah prosedur komputasional bertahap yang menerima masukan dan menghasilkan keluaran dalam waktu berhingga. Sintaks adalah aturan formal tata bahasa pemrograman tertentu (seperti titik koma atau tanda kurung kurawal). Sebuah kode program dapat valid secara sintaksis dan berhasil dikompilasi, tetapi mengalami logical error (kesalahan logika) jika relasi sebab-akibat antar-instruksi tidak memenuhi post-condition yang diharapkan.",
        "code_examples": [
            {
                "language": "typescript",
        ...
        "why_vibecoding_matters": "Saat vibecoding dengan asisten AI, model sering menghasilkan kode yang terlihat meyakinkan tetapi memiliki kekeliruan urutan operasi (misalnya mengosongkan token otentikasi sebelum permintaan HTTP selesai dikirim). Pahami alur logika agar kamu bisa meneliti urutan kejadian yang dihasilkan AI. Ajukan pertanyaan ini ke AI: 'Jelaskan diagram alur eksekusi langkah demi langkah dan pastikan prasyarat tiap operasi terpenuhi sebelum baris berikutnya dieksekusi.'",
        "keywords": [
            "logika",
            "urutan",
            "sequence",
        ...
        "sort_order": 1,
        "is_active": true,
        "problem_context": "Di masa awal perancangan perangkat elektronik, sirkuit kabel fisik harus dipindahkan secara manual untuk mengubah perhitungan matematika. Ketika arsitektur stored-program diperkenalkan oleh von Neumann, timbul masalah besar: bagaimana cara manusia merumuskan instruksi yang kompleks agar jutaan operasi logika dapat dijalankan tanpa kekacauan urutan? Tanpa logika pemrograman yang terstruktur, instruksi beruntun akan saling menimpa hasil perhitungan register prosesor, mengakibatkan data hilang dan hasil yang tidak dapat diprediksi.",
        "misconceptions": [
            {
        ...
        "when_to_use": "Terapkan pemikiran logika sekuensial dan formalisasi algoritma pada setiap modul software sebelum menulis sintaks. Gunakan diagram alir (flowchart) atau pseudocode saat memecah masalah bisnis yang memiliki banyak dependensi urutan. Hindari membuat alur logika yang terlalu panjang dalam satu blok tanpa pembagian tugas modular.",
    ...
    },
    {
        ...
        "explanation_technical": "Kondisional berakar pada instruksi CPU conditional branch (seperti JZ/Jump if Zero atau JNE/Jump if Not Equal) yang membaca status register flags prosesor (seperti Zero Flag ZF) setelah operasi perbandingan CMP. Jika kondisi terpenuhi, Program Counter diarahkan ke label alamat instruksi cabang target.\n\nKonstruksi tingkat tinggi mencakup struktur if, else if, else, serta switch-case. Praktek arsitektur modern sangat menganjurkan pola Guard Clauses (Early Return), di mana kondisi kegagalan atau validasi batas diperiksa dan dihentikan di baris-baris awal fungsi, sehingga logika utama tidak tertimbun di dalam piramida kurung kurawal (arrow anti-pattern). Konsep truthy dan falsy pada bahasa bertipe dinamis (seperti 0, '', null, undefined di JS) harus diwaspadai karena konversi implisit dapat meloloskan percabangan tak terduga.",
        "code_examples": [
            {
                "language": "typescript",
    ...
    },
    {
        ...
        "explanation_technical": "Perulangan diimplementasikan di CPU menggunakan kombinasi instruksi perbandingan nilai dan lompatan bersyarat kembali ke label awal blok kode (misalnya CMP counter, limit diikuti JLE loop_start). Tiga struktur perulangan standar meliputi: 1. for loop: digunakan ketika jumlah iterasi diketahui di awal (mengelola inisialisasi, kondisi terminasi, dan inkremen). 2. while loop: digunakan ketika pengulangan bergantung pada kondisi eksternal yang belum pasti kapan berubahnya (pre-check condition). 3. do-while loop: menjamin tubuh kode dieksekusi minimal satu kali sebelum kondisi diperiksa (post-check condition).\n\nDalam eksekusi loop, kata kunci break digunakan untuk keluar seketika dari perulangan, sedangkan continue melewati sisa blok iterasi saat ini dan melompat langsung ke evaluasi iterasi berikutnya. Kesalahan paling fatal pada loop adalah infinite loop (di mana variabel kondisi tidak pernah berubah menuju false) dan off-by-one error (perulangan kelebihan atau kekurangan satu langkah karena keliru menggunakan operator < versus <=).",
        "code_examples": [
            {
                "language": "typescript",
